...
MessageType: enum { 
  ...
  -- ISD cache_I_load,
  -- IMAD cache_I_store,
  -- IMA cache_I_store_GetM_Ack_AD,
  -- S cache_S,
  -- SMAD cache_S_store,
  -- SMA cache_S_store_GetM_Ack_AD,
  -- SIA cache_S_evict,
  -- M cache_M,
  -- MIA cache_M_evict,
  -- IIA cache_M_evict_Fwd_GetM,
...
OBJSET_cache1: enum{cache1, cache2, cache3, cache4};
...
Machines: union{OBJSET_cache1, OBJSET_directory0, OBJSET_directory1};
...
-- Modified due to ID
-- v_NrCaches_OBJSET_cache0: multiset[2] of OBJSET_cache0;
-- cnt_v_NrCaches_OBJSET_cache0: 0..1;
...
if ((msg.dst = cache0) & (msg.src = cache2) & (msg.adr = 1)) then
  ...
  elsif ((msg.dst = cache1) & (msg.src = cache3) & (msg.adr = 0)) then
    fwd[0][cnt_fwd[0]] := msg;
    cnt_fwd[0] := cnt_fwd[0] + 1;
...
switch inmsg.mtype
  case GetM:
    msg := RespAck(adr,GetM_Ack_AD,m,inmsg.src,cle.cl,VectorCount_v_NrCaches_OBJSET_cache1(cle.cache));
    Send_resp(msg);
    cle.owner := inmsg.src;
  ...
  case PutS:
    ...
    Send_fwd(msg);
    RemoveElement_v_NrCaches_OBJSET_cache1(cle.cache,inmsg.src);
    if (VectorCount_v_NrCaches_OBJSET_cache1(cle.cache)=0) then
    cle.State := directory_I;
    cle.Perm := none;
...
--           if IsMember(n, OBJSET_directory) then -- ismember should decide the id of marhicnes
